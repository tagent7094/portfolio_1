...
class OllamaProvider(LLMProvider):
    ...
    def generate_stream(
        ...
        """Stream tokens from Ollama. Prints to terminal in real-time."""
    ...
    def generate_json(self, prompt: str, system_prompt: str = None) -> dict:
        print(f"\033[36m[LLM:{self.model}]\033[0m generate_json() prompt={len(prompt)} chars", file=sys.stderr, flush=True)
        for attempt in range(3):
            if attempt > 0:
                ...
                prompt_to_use = prompt
            response = self.generate(prompt_to_use, system_prompt, temperature=0.3)
