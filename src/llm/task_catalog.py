...
_RAW_TASKS: list[TaskSpec] = [
    ...
    # ── Lean-mode batched tasks ──
    TaskSpec(
        task_id="generate_a_batch",
        display_name="Batch A — dissect + 3 mirrored posts",
        description="Lean mode: combines source dissection and 3 Batch-A posts into a single LLM call.",
        call_site="src/batch/pack_generator.py:_generate_a_batch",
        default_purpose="generation",
        quality_tier="heavy",
        frequency="per_source",
        default_temperature=0.5,
    ...
    TaskSpec(
        task_id="generate_b_batch",
        display_name="Batch B — 3 mechanics-only posts",
        description="Lean mode: generates 3 Batch-B posts with different entry doors in one call.",
        call_site="src/batch/pack_generator.py:_generate_b_batch",
        default_purpose="generation",
        quality_tier="heavy",
        frequency="per_source",
        default_temperature=0.5,
    ...
    TaskSpec(
        task_id="voice_validation_batch",
        display_name="Voice check — all 9 posts",
        description="Lean mode: validates voice markers and register fit for all posts in one call.",
        call_site="src/batch/voice_validator.py:validate_voice_batch",
        default_purpose="generation",
        quality_tier="heavy",
        frequency="per_source",
        default_temperature=0.3,
    ...
    TaskSpec(
        task_id="amplifier_batch",
        display_name="Amplify — diagnose + opener for all 9",
        description="Lean mode: runs 7-gate diagnosis and generates best opener for all posts in one call.",
        call_site="src/batch/amplifier.py:amplify_batch",
        default_purpose="generation",
        quality_tier="heavy",
        frequency="per_source",
        default_temperature=0.5,
