...
def get_deep_founder_context(graph: nx.DiGraph, platform: str) -> dict:
    """Deep founder inventory for internalization. Delegates to v2.

    Backwards-compat shim. The v2 returns additional keys (cast, scenes,
    milestones, top_beliefs_by_conviction, topic_neighbors) that existing
    callers ignore. New callers should use `get_deep_founder_context_v2`
    from `src.graph.conviction_query` directly when they need the extras.
    """
    _log(f"get_deep_founder_context(platform={platform!r}) — delegating to v2")
    from .conviction_query import get_deep_founder_context_v2
    return get_deep_founder_context_v2(graph, platform, topic=None)
